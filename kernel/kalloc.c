...
kfree(void *pa)
{
  ...
  acquire(&count_lock);
  int index = ((uint64)pa - KERNBASE) >> 12;
  if (ref_count[index] <= 0)
    panic("kfree: negative ref count");
  
  ref_count[index]--;    // decrement the reference count
  int after = ref_count[index];
  release(&count_lock);
...
kalloc(void)
{
  ...
  if(r) {
    ...
    acquire(&count_lock);
    ref_count[((uint64)r - KERNBASE) >> 12] = 1;    // initialize reference count to 1
    release(&count_lock);
  }
  return (void*)r;
}
